    ...
    if (extractedContent && proofSpec) {
      ...
      if (proofSpec.text_proof) {
        ...
        if (hasText) {
          ...
          const textMatch = textContent.toLowerCase().includes(proofSpec.text_proof.toLowerCase());
...
- text_proof: Check if extracted captions/audio contain the specified text or similar content. Be flexible with text extraction formatting issues (missing spaces, merged words, minor typos) that commonly occur with social media text extraction. Focus on semantic meaning rather than exact character matching.
...
SCORING: Each requirement should be evaluated independently. If text_proof is found, mark it as met. If platform matches, mark it as met. Calculate overall confidence based on requirements met vs. total requirements.
